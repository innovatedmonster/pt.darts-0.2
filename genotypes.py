...
def parse(alpha, k):
    ...
    # 2) Choose top-k edges per node by edge score (top-1 weight in edge), default k is 2
    for edges in alpha:
        """edges: Tensor(n_edges, n_ops), 
        is bidimensional alpha tensor, representing node's input alphas"""
        edge_max, primitive_indices = torch.topk(edges[:, :-1], 1) # ? ignore 'none',r open
        topk_edge_values, topk_edge_indices = torch.topk(edge_max.view(-1), k)
        node_gene = []
        for edge_idx in topk_edge_indices:
            prim_idx = primitive_indices[edge_idx]
            prim = PRIMITIVES[prim_idx]
            # node_gene is each nodes's 2 input edges, [(op1, index1),(op2, index2)]
